    ...
    def ensure_label(
        ...
        try:
            ...
            try:
                ...
                # Race condition: label was created concurrently by another process
                return repo.get_label(label_name)

    def fetch_active_fingerprints(self) -> set[str]:
        """Fetch 64-hex SHA-256 fingerprints from all open issues in target repository.
        ...
        try:
            ...
            logger.warning(
                "Could not fetch open repository issues for fingerprint deduplication: %s",
                e,
            )
