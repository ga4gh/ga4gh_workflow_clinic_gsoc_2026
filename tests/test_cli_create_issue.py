...
def test_create_issue_real_examine_output(tmp_path: Path) -> None:
    """Verify create-issue successfully parses diagnosis.json generated by workflow-clinic examine."""
    ...
    # Step 1: Run examine to produce real diagnosis.json
    examine_result = runner.invoke(app, ["examine", str(nf_file), "-o", str(diag_file)])
    assert examine_result.exit_code in (0, 1)
    assert diag_file.exists()

    # Step 2: Run create-issue against real examine output
